...
knitr::opts_chunk$set(eval = F, echo = TRUE, message = F, warning = F, comment = NA, out.width = "100%")
```
...
GWAS_PeakTable <- function(folder = NULL, file = NULL, 
  ...
  output <- output %>%
    mutate(Model = model, Trait = trait, Expt = expt,
           FT_Genes = NA, FT_Pos = NA, FT_End = NA, FT_Distance = NA, FT_Distances = NA)
  #
  if(sum(!is.na(output$Position)) > 0) {
    for(i in 1:nrow(output)) {
      ...
      ftg <- ftg %>% mutate(Distance = abs(Position - output$Position[i])) %>% 
        arrange(Distance)
...
#
write.csv(ww, "Supplemental_Table_01.csv", row.names = T)
```
...
gg_Custom_Man <- function(expttraits = c("Su17_DTF", "Ba17_DTF", "Ne17_DTF", "It17_DTF"),
                          ...
                          height = 8) {
  #
  vi <- read.csv("Results/GAPIT.MLM.Ro17_DTF.GWAS.Results.csv") %>% 
